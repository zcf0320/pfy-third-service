...
const FamilyDiseaseHistory = () => {
  ...
  const no = () => {
    setValue(false);
    HealthRecordStore.setHealthyFile({ familyDiseaseHistory: '' });
  };
  useEffect(() => {
    if (familyDiseaseHistory) {
      let selectPast = familyDiseaseHistory.split(',');
      let neverPast: any = [];
  ...
  }, [familyDiseaseHistory, pastList]);
  const setPastStr = () => {
    let familyDiseaseHistoryStr = '';
    pastList.forEach((item) => {
      item.select &&
        (familyDiseaseHistoryStr += `${familyDiseaseHistoryStr ? ',' : ''}${
          item.text
        }`);
    });
    HealthRecordStore.setHealthyFile({
      familyDiseaseHistory: familyDiseaseHistoryStr,
    });
    !familyDiseaseHistoryStr &&
      HealthRecordStore.delHealthyFile('familyDiseaseHistory');
  };
  return (
...
export default observer(FamilyDiseaseHistory);
